const menu = [
  ...
  let foodItems = ""; // once bos gelsin

  //menuyu foodMenu icerisine forEach ile eklenmesini sagladik.Menuyu yukaridaki diziden cektik.icerisinde style uyguladik.
  menu.forEach((item) => {
    foodItems +=`
  ...
  //buttonsItem adinda yeni bir dizi olusturduk map ile.join ile söz dizimi yani string hale getirdik.
const buttonItems = buttons.map((button)=>`<button id="${button.id}">${button.text}</button>`).join("");

btnContainer.innerHTML = buttonItems;

btnContainer.addEventListener("click", (e)=> {
    if (e.target.tagName === "BUTTON") {
      const id = e.target.id; //Olayın meydana geldiği öğenin adını alın
      ...
      `<div class="menu-items col-lg-6 col-sm-12">
        ...
        <div class="menu-info">
            ...
            <div class="menu-text">${item.desc}</div>
            <div class="menu-text">${item.category}
            </div>
        </div>
     </div>`
      ).join("");
